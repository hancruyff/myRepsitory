    ...
    public partial class Form5 : Form
    {
        ...
        private void comboBox3_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                conn.Open();
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandText = "select * from 운행시간표 where 역순번=" + Sstation.ToString() + " and 방향='" + Direction + "' and 열차번호="+comboBox3.SelectedItem.ToString();
                cmd.CommandType = CommandType.Text;
                cmd.Connection = conn;
                OleDbDataReader read = cmd.ExecuteReader();
                while (read.Read())
        ...
        private void comboBox5_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                conn.Open();
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandText = "select * from 예약좌석 where 운행날짜='" + dateTimePicker1.Value.ToString("yyyyMMdd") + "' and 역순번=" + Sstation.ToString() + " and 열차번호=" + comboBox3.SelectedItem.ToString() + " and 방향='" + Direction + "' and 시간='" + comboBox4.SelectedItem.ToString() + "' and 차량번호=" + comboBox5.SelectedItem.ToString();
                cmd.CommandType = CommandType.Text;
                cmd.Connection = conn;
                OleDbDataReader read = cmd.ExecuteReader();
                while (read.Read())
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                conn.Open();
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandText = "select count(*) from 예약현황 where 예약번호 like 'SM%'";
                cmd.CommandType = CommandType.Text;
                cmd.Connection = conn;
                OleDbDataReader read = cmd.ExecuteReader();
                while (read.Read())
                ...
                for (int i = S; i <= D; i++)
                {
                    for(int j = 0; j < 8; j++)
                    {
                        if (seat[j] == 1)
                        {
                            cmd.CommandText = "insert into 예약좌석 (역순번,역이름,열차번호,방향,시간,운행날짜,차량번호,좌석번호,예약번호) values(" + i.ToString() + ",(select 역이름 from 기차역 where 역순번=" + i.ToString() + ")," + comboBox3.SelectedItem.ToString() + ",'" + Direction + "','" + comboBox4.SelectedItem.ToString() + "','" + dateTimePicker1.Value.ToString("yyyyMMdd") + "'," + comboBox5.SelectedItem.ToString() + "," + j.ToString() + ",'" + RN + "')";
                            cmd.CommandType = CommandType.Text;
                            cmd.Connection = conn;
                            cmd.ExecuteNonQuery();
                        }
